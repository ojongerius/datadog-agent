...
// SliceBuilder is used to build tagsets tag-by-tag, before "freezing" into a Tags instance. SliceBuilders are not threadsafe.
//
...
func (bldr *SliceBuilder) reset(levels int) {
	bldr.levels = levels
	...
	// bldr.tags and bldr.hashes will be reallocated as necessary, so
	// they need only be truncated
