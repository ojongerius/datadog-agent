...
type RuleEngine struct {
	...
	opts      *Opts
	policy    *RuleSet
	profiles  map[*eval.Rule]*ActiveProfile
...
type RuleEngineListener interface {
	...
	ProfileUnmatch(profile *Profile, event eval.Event)
	EventDiscarderFound(re *RuleEngine, event eval.Event, field eval.Field, eventType eval.EventType)
}

// ActiveProfile describes a profile definition along with its corresponding rule set
...
func (re *RuleEngine) GetProfile(ctx *eval.Context) *ActiveProfile {
...
// NotifyProfileUnmatch notifies all the rule engine listeners that an could be matched with its profile
func (re *RuleEngine) NotifyProfileUnmatch(profile *Profile, event eval.Event) {
	for _, listener := range re.listeners {
		listener.ProfileUnmatch(profile, event)
	}
}
...
func (re *RuleEngine) Evaluate(event eval.Event) bool {
	...
	if profile := re.GetProfile(ctx); profile != nil {
		...
		if profileMatch {
			...
			re.NotifyProfileUnmatch(profile.Profile, event)
		}
	}
	...
	if profileMatch && !policyMatch {
		...
		for _, field := range bucket.fields {
			...
			if isDiscarder {
				...
				re.logger.Tracef("Found discarder for %s: %v", field, value)
...
// LoadProfiles loads a set of profiles into the rule engine
...
PROFILE:
	for _, profilePath := range profileFiles {
		...
		re.profiles[selectorRule] = &ActiveProfile{
			RuleSet: ruleSet,
			Profile: profile,
		}
	}
...
// GetPolicy returns the set of loaded policy rules
func (re *RuleEngine) GetPolicy() *RuleSet {
	return re.policy
}

// GetProfiles returns the loaded profiles
func (re *RuleEngine) GetProfiles() map[*eval.Rule]*ActiveProfile {
	return re.profiles
}

// NewRuleEngine returns a new rule engine for the specified model and event constructor
func NewRuleEngine(model eval.Model, eventCtor func() eval.Event, opts *Opts) *RuleEngine {
	return &RuleEngine{
		...
		profiles:  make(map[*eval.Rule]*ActiveProfile),
	}
}
