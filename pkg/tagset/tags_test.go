...
func TestTags_TagsString(t *testing.T) {
	tagset := newTags([]string{"foo", "bar"})
	require.Equal(t, tagset.String(), "foo, bar")
}

func TestTags_TagsMarshalDSD(t *testing.T) {
	tagset := newTags([]string{"foo", "bar"})
	require.Equal(t, tagset.MarshalDSD(), []byte("foo,bar"))
}

func TestTags_TagsMarshalDSDEmpty(t *testing.T) {
	tagset := newTags([]string{})
	require.Equal(t, tagset.MarshalDSD(), []byte{})
}

func TestTags_TagsHash(t *testing.T) {
	tagset := newTags([]string{"foo"})
	require.Equal(t, tagset.Hash(), fooHash)
}

func TestTags_TagsSorted(t *testing.T) {
	tagset := newTags([]string{"foo", "bar"})
