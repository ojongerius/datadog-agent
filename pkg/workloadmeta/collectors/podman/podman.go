...
func (c *collector) Start(_ context.Context, store workloadmeta.Store) error {
	if !config.IsFeaturePresent(config.Podman) {
		return dderrors.NewDisabled(componentName, "Agent is not running on Podman")
	}
...
func (c *collector) Pull(_ context.Context) error {
	...
	for _, container := range containers {
		...
		if err != nil {
			log.Warnf("Error getting info from Podman container: %s", err.Error())
			continue
		}
...
func convertToEvent(container *podman.Container) (workloadmeta.CollectorEvent, error) {
